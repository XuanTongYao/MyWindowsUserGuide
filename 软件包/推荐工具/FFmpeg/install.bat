...
IF /I "%3" EQU "Y" (
    echo "下载FFmpeg安装包中"
    @echo off
    powershell -Command "$url = 'https://www.gyan.dev/ffmpeg/builds/ffmpeg-git-full.7z'; $output = '.\'; $filename = [System.IO.Path]::GetFileName($url); $fullOutputPath = Join-Path -Path $output -ChildPath $filename; $webclient = New-Object System.Net.WebClient; $webclient.DownloadFile($url, $fullOutputPath)"
    EXIT
) 
SET Drive=%1
SET Type=%2
